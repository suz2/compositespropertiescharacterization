function overlap = overlap_ellipses(x0,y0,a0,b0,theta0,x1,y1,a1,b1,theta1,threshold,threshold2)
...
if NORM < b0 + b1 + threshold
    overlap = 1; 
else
    overlap = 0;
end
if NORM > a0 + a1 + threshold2; overlap = 0; end;
...
if overlap == -1;
    ...
    DIST = [X1-x0;Y1-y0]';
    NORM = sqrt(DIST(:,1).^2+DIST(:,2).^2);
    if min(NORM) < b1 + threshold
        overlap = 1;
    else
        overlap = 0;
    end
%     if min(NORM) > a1 + threshold2; overlap = 0; end;

    % Check distances for severe overlaps
    DIST = [X0-x1;Y0-y1]';
    NORM = sqrt(DIST(:,1).^2+DIST(:,2).^2);
    if min(NORM) < b0 + threshold
        overlap = 1;
    else
        overlap = 0;
    end
%     if min(NORM) > a0 + threshold2; overlap = 0; end;
end
