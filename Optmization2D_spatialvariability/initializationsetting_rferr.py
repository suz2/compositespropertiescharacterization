    ...
    for Eint_ind in np.arange(level):
        for alpha_ind in np.arange(level):
            for num_ind in np.arange((cpu_ind - 1) * level / num_cpu, cpu_ind * level / num_cpu, 1):
                os.system('cp optmechload_expmod_rferr.py '+directory+'/optmechload_expmod_e'+str(err_index)+\
...
##############################
f=open('optmechload_expmod_rferr.py','r+')
flist=f.readlines()
...
f=open('optmechload_expmod_rferr.py','w+')
f.writelines(flist)
f.close()
